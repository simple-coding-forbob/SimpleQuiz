...
export default function QuizPage() {
  const { menuId, chapterId, sectionId, quizId } = useParams();
  ...
  const checkAnswer = () => {
    ...
    else if (score >= Math.ceil(quiz.grading.length / 2))
      grade = "🟡 부분 정답";
    else grade = "❌ 다시 시도";
  ...
  // 선택한 챕터
  const chapter = menu.chapters.find((c) => c.chapterId === Number(chapterId));
  ...
  // ===== 이전/다음 이동 =====
  const goPrev = () => {
    if (currentIndex > 0 && Number(quizList[currentIndex - 1].quizId[1]) === section.sectionId)
      nav(`/${menu.menuId}/chapter/${chapter.chapterId}/section/${section.sectionId}/quiz/${quizList[currentIndex - 1].quizId}`);
  };
  const goNext = () => {
    if (currentIndex < quizList.length - 1  && Number(quizList[currentIndex + 1].quizId[1]) === section.sectionId)
      nav(`/${menu.menuId}/chapter/${chapter.chapterId}/section/${section.sectionId}/quiz/${quizList[currentIndex + 1].quizId}`);
  };

  if (!quiz || !quizMeta)
    return <div className="p-6">문제를 찾을 수 없습니다.</div>;
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex justify-center py-10">
      <div className="bg-white w-full max-w-3xl rounded-xl shadow-lg p-6">
        <h2 className="font-bold mb-4 bg-blue-50 p-5">
          {menu.title} - {chapter.title} - {section.title} - 퀴즈 {quizMeta.title}
        </h2>
        ...
        {/* 결과 */}
        {result && (
          <div className="mt-4 font-bold whitespace-pre-line text-left">
            {result}
          </div>
        )}
